...
class FTSensor(object, Node):
  ...
  """
    ...
    """
        ...
        self.create_subscription('%sft_sensor/diagnostics' % ns, DiagnosticArray, self.cb_diagnostics)
        self.create_subscription('%sft_sensor/raw' % ns, WrenchStamped, self.cb_raw)
        self.create_subscription('%sendpoint_state' % ns, EndpointState, self.cb_compensated)
        initime = rclpy.get_time()
        while not rclpy.is_shutdown() and not self.is_raw_alive():
            time.sleep(0.1)
            if (rclpy.get_time() - initime) > timeout:
                rclpy.logwarn('FTSensor: Cannot read raw wrench')
                return
        if read_compensated:
            initime = rclpy.get_time()
            while not rclpy.is_shutdown() and not self.is_compensated_alive():
                time.sleep(0.1)
                if (rclpy.get_time() - initime) > timeout:
                    self.get_logger().warn('FTSensor: Cannot read compensated wrench')
                    return
        self.get_logger().info('FTSensor successfully initialized')
...
class FTSensorSynch(Node):
    ...
    def __init__(self, namespace='', compensated=False, timeout=5.0):
        ...
        while np.isclose(rclpy.get_time(), 0):
            time.sleep(0.001)
        # Set-up subscribers
        ...
        if not self.wait_for(self.is_msg_recent, args=(lambda: self.diagnostics_msg, 1.5), timeout=timeout):
            self.get_logger().warn('FTSensor: Cannot read sensor diagnostics')
            return
        if not self.wait_for(self.is_msg_recent, args=(lambda: self.raw_msg, 2.0 / self.rate), timeout=timeout):
            self.get_logger().warn('FTSensor: Cannot read raw wrench')
            return
        if compensated:
            self.create_subscription('%sendpoint_state' % ns, EndpointState, self.cb_compensated)
            if not self.wait_for(self.is_msg_recent, args=(lambda: self.compensated_msg, 2.0 / self.rate),
                                 timeout=timeout):
                self.get_logger().warn('FTSensor: Cannot read compensated wrench')
                return
    ...
    def wait_for(self, predicate, args, timeout=5.0):
        ...
        while not rclpy.is_shutdown() and not predicate(*args):
            now = rclpy.get_time()
